...
RUN	git clone https://github.com/komodoplatform/komodo && cd komodo/src && ./../zcutil/fetch-params.sh && \
	...
	cd ../.. && rm -rf komodo
RUN	apt-get install -y nano && apt-get remove -y curl wget build-essential pkg-config libc6-dev m4 g++-multilib autoconf libtool ncurses-dev unzip git python python-zmq zlib1g-dev wget libcurl3-gnutls-dev bsdmainutils automake
